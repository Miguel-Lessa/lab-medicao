...
if not TOKEN:
    raise ValueError("GITHUB_TOKEN nao encontrado. Defina em .env")
...
def collect_top_repositories(total: int) -> tuple[list[dict], datetime]:
    ...
    while len(repos) < total:
        ...
        for node in nodes:
            ...
            updated_at = parse_iso_utc(node["updatedAt"])
...
def build_summary(repos: list[dict]) -> dict:
        ...
        language_breakdown.append(
            {
                ...
                "median_days_since_update": round(median([r["days_since_update"] for r in group]), 2),
            }
        )
    ...
    return {
        ...
        "rq04_median_days_since_update": round(median([repo["days_since_update"] for repo in repos]), 2),
...
def render_rq07_table(rows: list[dict], top_n: int = 15) -> str:
    ...
    for row in rows[:top_n]:
        lines.append(
            ...
            f"{row['median_releases']} | {row['median_days_since_update']} |"
        )
    return "\n".join(lines)
...
- RQ04 (tempo ate ultima atualizacao): mediana de **{summary['rq04_median_days_since_update']} dias**.
